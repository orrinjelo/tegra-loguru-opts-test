...
// ----------------------------------------------------------------------------
	...
	#define LOGURU_THREADNAME_WIDTH 10
#endif
...
#undef MAX
	...
	*/
	...
	const char* terminal_get_color(Verbosity);
...
namespace loguru
{
	...
	const char* terminal_bright_blue(){ return s_terminal_has_color ? "\e[1;34m" : ""; }
	...
	const char* terminal_get_color(Verbosity v)  { 
		switch(v)
		{
			case Verbosity_TRACE:
			case Verbosity_VOMIT:
				return terminal_purple();
			case Verbosity_FILE:
				return terminal_bright_blue();
	...
	static void parse_args(int& argc, char* argv[], const char* verbosity_flag)
	{
		...
		for (int arg_it = 1; arg_it < argc; ++arg_it) {
			...
			if (strncmp(cmd, verbosity_flag, arg_len) == 0 && !std::isalpha(cmd[arg_len], std::locale(""))) {
				...
				} else if (strcmp(value_str, "file") == 0) {
					g_stderr_verbosity = Verbosity_FILE;
				...
				} else if (strcmp(value_str, "debug") == 0) {
					g_stderr_verbosity = Verbosity_DEBUG;
				} else if (strcmp(value_str, "vomit") == 0) {
					g_stderr_verbosity = Verbosity_VOMIT;
				} else if (strcmp(value_str, "trace") == 0) {
					g_stderr_verbosity = Verbosity_TRACE;
				} else {
					char* end = 0;
					g_stderr_verbosity = static_cast<int>(strtol(value_str, &end, 10));
					CHECK_F(end && *end == '\0',
						"Invalid verbosity. Expected integer, vomit, trace, info, debug warning, error, fatal, or OFF, got '%s'", value_str);
				}
			} else {
	...
	static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line)
	{
		...
		} else if (verbosity == Verbosity_VOMIT) {
			snprintf(level_buff, sizeof(level_buff) - 1, "VMIT");
		} else if (verbosity == Verbosity_TRACE) {
			snprintf(level_buff, sizeof(level_buff) - 1, "TRCE");
		} else if (verbosity == Verbosity_DEBUG) {
			snprintf(level_buff, sizeof(level_buff) - 1, "DBUG");
		} else if (verbosity == Verbosity_INFO) {
			snprintf(level_buff, sizeof(level_buff) - 1, "INFO");
		} else if (verbosity == Verbosity_FILE) {
			snprintf(level_buff, sizeof(level_buff) - 1, "FILE");
		} else {
			snprintf(level_buff, sizeof(level_buff) - 1, "% 4d", verbosity);
		}

		snprintf(out_buff, out_buff_size, "%04d-%02d-%02d %02d:%02d:%02d.%03lld (%8.3fs) [%-*s]%*s:%-5u %4s| ",
			// "%04d-%02d-%02d %02d:%02d:%02d.%03lld (%4.3fs) %*s[%08x] %*s:%-5u %4s| ",
			1900 + time_info.tm_year, 1 + time_info.tm_mon, time_info.tm_mday,
			time_info.tm_hour, time_info.tm_min, time_info.tm_sec, ms_since_epoch % 1000,
			uptime_sec,
			...
			if (g_colorlogtostderr && s_terminal_has_color) {
				if (verbosity > Verbosity_WARNING) {
					fprintf(stderr, "%s%s%s%s%s%s%s%s%s\n",
						terminal_reset(),
						terminal_get_color(verbosity),
						message.preamble,
						message.indentation,
						terminal_reset(),
						terminal_reset(), //Verbosity_INFO ? terminal_bold() : terminal_light_gray(),
						message.prefix,
						message.message,
						terminal_reset());
				} else {
